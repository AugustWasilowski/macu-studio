...
# === PATCH (local fix): POST /gallery/voices upload-and-save shim ===
# The shipped 0.3.x SPA POSTs new voice uploads to /gallery/voices and reads
# the resulting voice list from the same path. The upstream backend only
# implements GET /gallery/voices (search) and POST .../save-as-profile. This
# shim accepts the multipart upload, writes it directly into voice_profiles
# (same code path as POST /profiles), and returns a gallery-voice-shaped dict
# so the SPA's caller is happy. Once upstream ships the matching backend, we
# can drop this file and the bind-mount.
import uuid as _uuid
import time as _time
import os as _os
from fastapi import File as _File, Form as _Form, UploadFile as _UploadFile
from core.config import VOICES_DIR as _VOICES_DIR
from core.db import db_conn as _db_conn


@router.post("/gallery/voices")
async def _patch_create_voice(
    audio: _UploadFile = _File(...),
    name: str = _Form(...),
    ref_text: str = _Form(""),
    style: str = _Form(""),
    language: str = _Form("Auto"),
):
    profile_id = _uuid.uuid4().hex[:8]
    ext = _os.path.splitext(audio.filename or ".wav")[1] or ".wav"
    audio_filename = f"{profile_id}{ext}"
    audio_path = _os.path.join(_VOICES_DIR, audio_filename)
    with open(audio_path, "wb") as f:
        f.write(await audio.read())
    try:
        with _db_conn() as conn:
            conn.execute(
                ...
                (profile_id, name, audio_filename, ref_text, style, language, None, "", _time.time()),
            )
    except Exception:
