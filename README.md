...
| `skills/` | Claude Code agent skills — the render driver (`macu-render`), the publish driver (`macu-publish`, render→YouTube→macu-web), channel setup (`setup-macu-channel`), the CoWork harvest skill (`cowork-harvest`, for a browser-capable agent to work the free-web → local-harvest job-board), and `higgsfield-web-free-gen` (drive the Higgsfield web app for $0 generations under the Beta unlimited entitlements, avoiding the UI traps that silently bill). Show-specific authoring skills are not shipped; bring your own. |
