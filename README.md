...
| `skills/` | Claude Code agent skills — the render driver (`macu-render`), the publish driver (`macu-publish`, render→YouTube→macu-web), channel setup (`setup-macu-channel`), and the CoWork harvest skill (`cowork-harvest`, for a browser-capable agent to work the free-web → local-harvest job-board). Show-specific authoring skills are not shipped; bring your own. |
