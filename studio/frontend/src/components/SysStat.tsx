...
export function SysStat() {
  ...
  const mem =
    s?.gpu_mem_used_mib != null && s?.gpu_mem_total_mib != null
      ...
      : "—";
  return (
    <div className="flex items-center gap-2" title={t("sysstat.outerTooltip")}>
      <span className="seg-readout">
        {t("sysstat.cpu")} <span style={hot(s?.cpu_pct)}>{pct(s?.cpu_pct)}</span>
      </span>
      <span className="seg-readout cyan">
        {t("sysstat.gpu")} <span style={hot(s?.gpu_pct)}>{pct(s?.gpu_pct)}</span>
        <span className="text-txt-faint"> · </span><span className="text-amber">{mem}</span>
      </span>
      <span className="seg-readout" title={t("sysstat.diskTooltip", { dev: s?.disk_dev ?? "?" })}>
        {t("sysstat.dsk")} <span style={reading(s?.disk_read_mibps)}>{mbps(s?.disk_read_mibps)}↓</span>
        <span className="text-txt-faint"> {mbps(s?.disk_write_mibps)}↑ {t("sysstat.mbps")}</span>
      </span>
    </div>
  );
}
