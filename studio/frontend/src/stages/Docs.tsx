...
export function Docs() {
  ...
  const listQ = useQuery({ queryKey: ["docs"], queryFn: docsApi.list });
  ...
  useEffect(() => {
    if (!name && docs.length) setName(docs[0].name);
  }, [docs, name]);

  const docQ = useQuery({
    queryKey: ["doc", name],
    queryFn: () => docsApi.get(name),
    enabled: !!name,
  });

  useEffect(() => {
    if (docQ.data && docQ.data.name === name) {
  ...
  const saveMut = useMutation({
    mutationFn: () => docsApi.put(name, text),
    onSuccess: () => {
      setSaved(true);
      push(`${name} saved`, "ok");
      qc.invalidateQueries({ queryKey: ["docs"] });
      qc.invalidateQueries({ queryKey: ["doc", name] });
    },
    ...
    <div className="grid grid-cols-[280px_1fr] gap-3 h-full min-h-0">
      ...
      <section className="panel flex flex-col min-h-0">
        <header className="px-3 py-2 border-b hairline">
          <div className="panel-title">CANON DOCS <span className="text-txt-faint normal-case tracking-normal text-[11px]">/ docs/</span></div>
        </header>
        <div className="flex-1 overflow-y-auto p-2 flex flex-col gap-1">
          ...
          {docs.map((d: DocSummary) => {
            ...
            return (
              ...
              >
                <div className="font-mono text-[12px] truncate">{d.name}</div>
                <div className="label-tiny text-txt-faint">{fmtBytes(d.bytes)}</div>
              </button>
            );
          })}
          {!listQ.isLoading && docs.length === 0 && (
            <div className="text-txt-faint p-2">No docs in docs/.</div>
          )}
        </div>
      </section>
      ...
      <section className="panel flex flex-col min-h-0">
        <header className="flex items-center justify-between px-3 py-2 border-b hairline">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              className="btn"
              disabled={saved || !name || saveMut.isPending}
        ...
        <div className="flex-1 min-h-0">
          {!name ? (
            ...
            <textarea
              ...
              onBlur={() => { if (!saved && name) saveMut.mutate(); }}
            />
          )}
        </div>
