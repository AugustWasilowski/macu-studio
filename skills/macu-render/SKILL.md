...
- **Acquiring new SFX**: two paths, in order of preference:
  1. **freesound.org CC0** via `python3 /mnt/storage/shares/MACU/pipeline/freesound_fetch.py "<query>" <basename> [--duration-max N]`. Helper searches the freesound API, filters by CC0 license, downloads the top match's high-quality preview MP3, and normalizes it to the kit standard (24 kHz mono PCM s16 at −3 dBFS). Drops result into `assets/sfx/<basename>.wav`. Credentials at `~/.config/freesound/credentials.env` (mode 600 — `FREESOUND_CLIENT_ID` + `FREESOUND_API_KEY`). After accepting a file, append a catalog row to `assets/sfx/README.md` with the freesound URL + sound id + CC0 license.
  2. **ffmpeg lavfi synth** in the kit's own PD-synth style — see `assets/sfx/README.md` for the seed parameters used by the existing kit. Synthesis stays unambiguously public-domain and reproducible. Use this fallback for sounds with no good CC0 match on freesound, or when you want predictable in-mix character (the EBS tones, cricket chirp, etc. were synthesized this way).
